...
from src.service.processor.v3.LSTMProcessor2 import LSTMProcess
...
class OverseeServiceTest(unittest.TestCase):
    ...
    batch_code = "20250210001_600010.SH"
    labels = ['高股息精选']

    def test_model_with_all(self):
        oversee = OverseeService(batch_code=self.batch_code)
        oversee.set_single_stock_by_code(stock_code='600010.SH')
        df = oversee.load_data_with_label(start_date='20200101', end_date='20250207')
        ...
        self.predict(df=df, processor=processor, ts_code="600010.SH", start_date='2024-07-01', end_date='2025-01-05')

    def test_load_model_and_predict(self):
        oversee = OverseeService(batch_code=self.batch_code)
        oversee.set_single_stock_by_code("600010.SH")
        df = oversee.load_data_with_label(start_date='20200101', end_date='20250207')
        df['trade_date_copy'] = pd.to_datetime(df['trade_date'], format='%Y%m%d')

        processor = ProcessorBuilder.build_by_batch_code(batch_code=self.batch_code)

        self.predict(df=df, processor=processor, ts_code="600010.SH", start_date='2024-06-15', end_date='2024-11-05')
    ...
    def predict(self, df: pd.DataFrame, processor: Processor, ts_code: str, start_date: str, end_date: str):
        ...
        for start in range(start, end, 1):
            a_data = df_last.copy()
            a_period_data = a_data.iloc[start: start + processor.config.n_timestep]
            if len(a_period_data) < processor.config.n_timestep:
                break

            new_process = ProcessorBuilder.build_by_batch_code(batch_code=self.batch_code)
            a_predict_data = new_process.predict(a_period_data).future_predict_value
        ...
        for i in range(len(predict_data_line)):
            fig.add_trace(go.Scatter(x=last_trade_date_array, y=predict_data_line[i], mode='lines', name=f'Predict Price Line {i}', visible="legendonly", line=dict(dash='dash')))
