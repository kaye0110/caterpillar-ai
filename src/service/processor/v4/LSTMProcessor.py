...
from tensorflow.keras.layers import LSTM, GRU, Dense, Dropout, Bidirectional, Conv1D, MaxPooling1D, BatchNormalization
...
class LSTMProcess(Processor):
    ...
    def prepare(self, data: pd.DataFrame, ) -> "LSTMProcess":
        ...
        # 拟合 selector
        selector = RandomForestRegressor(n_estimators=self.config.n_estimators, max_depth=self.config.max_depth[0], random_state=self.config.random_state, n_jobs=self.config.n_jobs)
        ...
        scaled_data = self.feature_scaler.fit_transform(data[self.selected_features + [self.config.target_names[0]]])
    ...
    def build_model(self, hp):
        ...
        # 使用Conv1D和MaxPooling1D
        # model.add(Conv1D(filters=hp.Int('filters', min_value=128, max_value=512, step=32),
        #                  kernel_size=3, activation='relu', input_shape=input_shape))
        # model.add(MaxPooling1D(pool_size=2))
        # model.add(Dropout(hp.Float('dropout_rate_1', min_value=0.1, max_value=0.5, step=0.05)))
        #
        # # 使用GRU层
        # model.add(Bidirectional(GRU(units=hp.Int('units_gru', min_value=64, max_value=512, step=32), return_sequences=True)))
        # model.add(Dropout(hp.Float('dropout_rate_2', min_value=0.1, max_value=0.5, step=0.05)))
        #
        # # 使用LSTM层
        # model.add(Bidirectional(LSTM(units=hp.Int('units_lstm', min_value=256, max_value=512, step=32), return_sequences=False)))
        # model.add(Dropout(hp.Float('dropout_rate_3', min_value=00.1, max_value=0.5, step=0.05)))
        #
        ...
        model.add(Conv1D(filters=hp.Int('filters', min_value=64, max_value=312, step=32),
                         kernel_size=3, activation='relu', input_shape=input_shape,
        ...
        # GRU layer
        model.add(Bidirectional(GRU(units=hp.Int('units_gru', min_value=64, max_value=312, step=32), return_sequences=True)))
        model.add(BatchNormalization())
        model.add(Dropout(hp.Float('dropout_rate_2', min_value=0.1, max_value=0.5, step=0.05)))

        # LSTM layer
        model.add(Bidirectional(LSTM(units=hp.Int('units_lstm', min_value=64, max_value=312, step=32), return_sequences=False)))
        model.add(BatchNormalization())
        model.add(Dropout(hp.Float('dropout_rate_3', min_value=0.1, max_value=0.5, step=0.05)))

        # Output layer
        model.add(Dense(units=self.config.n_predict, activation='linear'))
    ...
    def train(self) -> "LSTMProcess":
        ...
        tuner = BayesianOptimization(
            ...
            project_name=self.config.batch_code + '_lstm_tuning_v4'
        )
        ...
        for train_index, test_index in tscv.split(self.X):
            ...
            tuner.search(X_train, y_train, epochs=self.config.epochs, validation_data=(X_test, y_test), callbacks=[early_stopping, reduce_lr], verbose=1)
